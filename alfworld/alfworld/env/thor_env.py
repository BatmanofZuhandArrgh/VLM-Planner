...
class ThorEnv(Controller):
    ...
    def save_frames(self, event):
        def save_png(e):
            ...
            #EDITED for path
            hardcoded_img_dir = os.path.join('../out',self.save_frames_path.split('alfworld/')[-1])
            os.makedirs(hardcoded_img_dir, exist_ok=True)

            cv2.imwrite(hardcoded_img_dir + '/%09d.png' % img_idx, rgb_image)
